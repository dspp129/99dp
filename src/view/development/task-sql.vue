    ...
    <div class="demo-tabs-style2">
      <Tabs v-model="tabStep" :animated="false" type="card" :capture-focus="true">
        ...
        <TabPane label="执行SQL" name="step1" :disabled="showController && maxStep < 2">
          ...
          <SQL2 v-model="dwTaskSQL.sql"
            :id="dwTask.jobId"
            :dbType.sync="dwTaskSQL.targetDbType"
            :connectionId.sync="dwTaskSQL.targetConnectionId" />
        </TabPane>
        <TabPane label="参数设计" name="step2" :disabled="showController && maxStep < 3">
          <StepController v-show="showController" v-model="step" />
          <Param v-model="dwTaskParam" />
        </TabPane>
        <TabPane label="周期依赖" name="step3" :disabled="showController && maxStep < 4">
...
export default {
  ...
  data () {
    return {
      showSpin: false,
      pageName: 'task-SQL',
      step: { length: 4, current: 0 },
      tabStep: 'step0',
      maxStep: 0,
      stepList,

      dwTask: JSON.parse(JSON.stringify(initData.initTask)),
      dwTaskSQL: JSON.parse(JSON.stringify(initTaskSQL)),
      dwTaskSlot: [],
      dependenceList: [],
      dwTaskParam: [],
      dwRecordHistory: [],
  ...
  methods: {
    ...
    async getTask () {
      ...
      if (item) {
        ...
        this.dwTaskSQL = item.task.dwTaskSQL
