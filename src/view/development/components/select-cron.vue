  ...
  <Modal
    ...
    <Row type="flex" justify="center" align="middle" >
      <select @change="changeCron" size="8" multiple="multiple" style="width:75px;" v-model="cronYear">
        ...
        <option v-for="i in 20" :value="i + 2017" :key="i">{{i + 2017}}年</option>
      </select>
      ...
      <select @change="changeCron" size="8" multiple="multiple" style="width:75px;" v-model="cronSecond">
        ...
        <option v-for="i in 59" :value="i" :key="i">{{i}}秒</option>
      </select>
    </Row>
    <Row type="flex" justify="center" align="middle" class="margin-top-10">
      <div style="width:525px;">
        <Col span="16">
          <Input size="large" placeholder="选择上方时钟与分钟" v-model="cronExpr" />
        </Col>
        <Col span="4">
...
export default {
  ...
  methods: {
    ...
    async checkCronExpr () {
      ...
      this.visible = false
      this.runTimeList = []
      const result = await taskApi.checkCronExpr({cronExpr: this.cronExpr})
      this.checking = false
      if (result.code === 0) {
